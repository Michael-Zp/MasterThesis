...
bool ShapesDemo::Init()
{
	...
	generator.CreateBox(1, 1, 1, meshes[0]);
	...
	XMStoreFloat4x4(&mObjectParams[0].WorldMatrix, XMMatrixTranslation(0, 0, 0));
	...
	for (int i = 0; i < meshes.size(); i++)
	{
		...
		{
			vertexData.push_back({ meshes[i].Vertices[k].Position, meshes[i].Vertices[k].TexC });
		}
	...
	mVertexShader->addVertexDesc("TEXTURE", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 12, 0);
	...
	mTexture = new Texture(L"./Textures/WoodCrate01.dds", md3dDevice);
...
void ShapesDemo::DrawScene()
{
	...
	mTexture->Activate(md3dImmediateContext);
