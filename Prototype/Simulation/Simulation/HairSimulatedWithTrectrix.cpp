...
HairSimulatedWithTrectrix::HairSimulatedWithTrectrix(ID3D11Device *device, ID3D11DeviceContext *context)
{
	...
	mSimulation = new TractrixSimulation(points, device, context);
	...
	mVertexShader = new VertexShader(L"./Shader/vgpHairTrectrixSimulated.hlsl", "HairVS", true);
	mVertexShader->prepare(device);

	mGeometryShader = new GeometryShader(L"./Shader/vgpHairTrectrixSimulated.hlsl", "HairGS", true);
	mGeometryShader->prepare(device);

	mPixelShader = new PixelShader(L"./Shader/vgpHairTrectrixSimulated.hlsl", "HairPS", true);
	mPixelShader->prepare(device);
}


HairSimulatedWithTrectrix::~HairSimulatedWithTrectrix()
{
	free(mSimulation);
}

void HairSimulatedWithTrectrix::Draw(const float deltaTime, ID3D11DeviceContext *context)
{
	if (!mIsUpdated)
	{
		DebugBreak();
...
void HairSimulatedWithTrectrix::UpdateCamera(ID3D11DeviceContext *context, XMMATRIX view, XMMATRIX proj)
{
