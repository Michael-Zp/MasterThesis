...
TractrixStepReturn TractrixStep(float3 tailPos, float3 headPos, float3 desiredHeadPos, float desiredSegmentLength)
{
...
void RecursiveTractrix(int idx, float3 Xp, float reverse, out float3 particleMovement[MAX_PARTICLE_COUNT], float oldSegmentLength[MAX_PARTICLE_COUNT - 1])
{
    ...
    */
    ...
    for (int i = 1; i <= strands[idx].ParticlesCount - 1; i++)
    {
        ...
        tractrixResult = TractrixStep(X, Xh, Xp, oldSegmentLength[i]);
...
void Simulation(uint3 DTid : SV_DispatchThreadID)
{
    ...
    for (int i = 0; i < MAX_PARTICLE_COUNT; i++)
    {
        ...
        if (i > 0)
        {
            strands[idx].Particles[i].Position = strands[idx].Particles[i - 1].Position + normalize(strands[idx].Particles[i].Position - strands[idx].Particles[i - 1].Position) * oldSegmentLength[i - 1];
        }
    }
    ...
    if (doTractrix && !(stopIfKnotChanged && strands[idx].KnotHasChangedOnce))
    {
        ...
        RecursiveTractrix(idx, strands[idx].HairRoot, 1.0, tractrixParticleMove, oldSegmentLength);
