...
void Simulation(uint3 DTid : SV_DispatchThreadID)
{
    ...
    */
    ...
    //if (strands[idx].SimulatedTime < 1)
    //{
    //    strands[idx].SimulatedTime += deltaTime;
    //    Xp = float3(sin(strands[idx].SimulatedTime), -1.25 - strands[idx].SimulatedTime, 0);
    //}
    //else
    //{
    //    Xp = float3(sin(strands[idx].SimulatedTime - 1), -1.25 - (strands[idx].SimulatedTime - 1), 0);
    //}
    ...
    //tempPos = float3(xr_n, yr_p, 0);
    //tempPos = float3(xr_np_p, yr_p, 0);
    //tempPos = float3(xr_np_n, yr_p, 0);
    //tempPos = float3(xr_p, yr_n, 0);
    //tempPos = float3(xr_n, yr_n, 0);
    //tempPos = float3(xr_np_p, yr_n, 0);
    ...
    //float3 oldPos_1 = strands[idx].Particles[i - 1].Position;
    //float3 oldPos = strands[idx].Particles[i].Position;
    
    strands[idx].Particles[i - 1].Position = newPos0;
    strands[idx].Particles[i].Position = Xp;
    
    //strands[idx].Particles[i - 1].Position = oldPos_1;
    //strands[idx].Particles[i].Position = oldPos;
    
    
    if (newPos0.x > 0)
    {
        //strands[idx].Particles[i - 1].Color = float3(0, 0, 1);
    }
    else
    {
        //strands[idx].Particles[i - 1].Color = float3(1, 0, 0);
    }
}