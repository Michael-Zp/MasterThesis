...
float3 cubicSplineInterpolation(float3 c[4], float ts[4], float t)
{
...
float3 multipleCubicSplineInterpolation(float3 c[4], float ts[4], float t)
{
    float3 q01 = cubicSplineInterpolationStep(c[0], c[1], ts[0], ts[1], t);
    float3 q11 = cubicSplineInterpolationStep(c[1], c[2], ts[1], ts[2], t);
    float3 q21 = cubicSplineInterpolationStep(c[2], c[3], ts[2], ts[3], t);
...
float3 cubicSpline(uint vertexId)
{
    ...
    //int cubicPartIdx = (int) floor(clamp(10.0 / vertexCount * strands[strandIdx].ParticlesCount, 0, strands[strandIdx].ParticlesCount - 1));
    ...
    //int knotLength = strands[strandIdx].ParticlesCount + 3;
    //float knot[MAX_PARTICLE_COUNT + 4];
    //knot[0] = 0;
    //knot[1] = 0;
    //knot[2] = 0;
    //knot[3] = 0;
    //knot[4] = 0;
    //for (int i = 5; i < knotLength - 3; i++)
    //{
    //    knot[i] = i - 3;
    //}
    //knot[knotLength - 4] = strands[strandIdx].ParticlesCount - 4;
    //knot[knotLength - 3] = strands[strandIdx].ParticlesCount - 4;
    //knot[knotLength - 2] = strands[strandIdx].ParticlesCount - 4;
    //knot[knotLength - 1] = strands[strandIdx].ParticlesCount - 4;
    
    
    //float currentKnot = knot[cubicPartIdx + 3];
    ...
    //float verticiesPerPart = vertexCount / strands[strandIdx].ParticlesCount;

    //if(vertexId / vertexCount < 0.4)
    //{
    //    currentKnot = 0;
    //}
    //else
    ...
    //float t = (vertexId - verticiesPerPart * currentKnot) / (verticiesPerPart * 4);
        
    //cs[0] = strands[strandIdx].particles[0 + currentKnot].Position;
    //cs[1] = strands[strandIdx].particles[1 + currentKnot].Position;
    //cs[2] = strands[strandIdx].particles[2 + currentKnot].Position;
    //cs[3] = strands[strandIdx].particles[3 + currentKnot].Position;
        
    //return cubicSplineInterpolation(cs, ts, t);
    
    //#########################
    //Again something
    
    //int strandIdx = (int) floor(vertexId / vertexCount);
    //int cubicPartIdx = clamp((int) floor((vertexId / vertexCount) * strands[strandIdx].ParticlesCount), 0, strands[strandIdx].ParticlesCount - 4);
    
    //int knotLength = strands[strandIdx].ParticlesCount + 4;
    //float knot[MAX_PARTICLE_COUNT + 4];
    //knot[0] = 0;
    //knot[1] = 0;
    //knot[2] = 0;
    //knot[3] = 0;
    //for (int i = 4; i < knotLength - 5; i++)
    //{
    //    knot[i] = i - 3;
    //}
    //knot[knotLength - 4] = strands[strandIdx].ParticlesCount - 3;
    //knot[knotLength - 3] = strands[strandIdx].ParticlesCount - 3;
    //knot[knotLength - 2] = strands[strandIdx].ParticlesCount - 3;
    //knot[knotLength - 1] = strands[strandIdx].ParticlesCount - 3;
    ...
    //float verticiesPerPart = strands[strandIdx].ParticlesCount / vertexCount;
    //float verticiesPerCicle = verticiesPerPart * 4;
    
    //float t = ((vertexId / vertexCount) - verticiesPerPart * currentKnot) / verticiesPerCicle;
