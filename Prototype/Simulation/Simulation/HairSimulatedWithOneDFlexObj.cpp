...
HairSimulatedWithOneDFlexObj::HairSimulatedWithOneDFlexObj(ID3D11Device *device, ID3D11DeviceContext *context)
{
	std::vector<std::vector<XMFLOAT3>*> points;
	points.push_back(new std::vector<XMFLOAT3>);
	points[0]->push_back(XMFLOAT3(0, 1.25, 0));
	points[0]->push_back(XMFLOAT3(0, 0, 0));
	points[0]->push_back(XMFLOAT3(0, -1.25, 0));

	mSimulation = new OneDFlexObjSimulation(points, device, context);
	...
	mVertexShader = new VertexShader(L"./Shader/vgpHairOneDFlexObjSimulated.hlsl", "HairVS", true);
	mVertexShader->prepare(device);

	mGeometryShader = new GeometryShader(L"./Shader/vgpHairOneDFlexObjSimulated.hlsl", "HairGS", true);
	mGeometryShader->prepare(device);

	mPixelShader = new PixelShader(L"./Shader/vgpHairOneDFlexObjSimulated.hlsl", "HairPS", true);
	mPixelShader->prepare(device);
}


HairSimulatedWithOneDFlexObj::~HairSimulatedWithOneDFlexObj()
{
	free(mSimulation);
}

void HairSimulatedWithOneDFlexObj::Draw(const float deltaTime, ID3D11DeviceContext *context)
{
	if (!mIsUpdated)
	{
		DebugBreak();
...
void HairSimulatedWithOneDFlexObj::UpdateCamera(ID3D11DeviceContext *context, XMMATRIX view, XMMATRIX proj)
{
